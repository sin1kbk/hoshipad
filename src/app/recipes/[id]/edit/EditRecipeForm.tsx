...
export default function EditRecipeForm({ recipe }: EditRecipeFormProps) {
  ...
  const [source, setSource] = useState<RecipeSource>(recipe.source)
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      if (data.suggestedCategory) {
        setTags([...tags, data.suggestedCategory].filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('レシピの更新に失敗しました')
